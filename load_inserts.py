...
DBname = "postgres"
DBuser = "postgres"
DBpwd = "godofwar97"   # insert your postgres db password here
...
def load(conn, filename):
	with conn.cursor() as cursor:
		print(f"Loading data using copy_from() from: {filename}")
		start = time.perf_counter()
...
def load(conn, filename):
	...
	# Preprocess CSV: replace empty strings with 0
	with open(filename, 'r') as infile, open(cleaned_file, 'w') as outfile:
		header = infile.readline()
		outfile.write(header)  # keep header
	...
	with conn.cursor() as cursor:
		print(f"Loading data using copy_from() from: {cleaned_file}")
		start = time.perf_counter()
